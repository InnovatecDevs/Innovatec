...
export class PerfilComponent implements OnInit {
  ...
  getProfilePhotoUrl(): string {
    if (this.photoPreview) {
      // Si es base64 o url temporal
      if (this.photoPreview.startsWith('data:')) return this.photoPreview;
      // Si es una ruta relativa guardada en el backend
      return 'http://localhost:3000/' + this.photoPreview;
    }
    if (this.user && this.user.fotoPerfil) {
      return 'http://localhost:3000/' + this.user.fotoPerfil;
    }
